...
public:
	...
	virtual ~Effect();
	ID3DX11EffectTechnique *GetTechniqueByName(const char *techName);
...
public:
	...
	void SetLayerMapArraySRV(ID3D11ShaderResourceView* tex) { LayerMapArray->SetResource(tex); }
	void SetBlendMapSRV(ID3D11ShaderResourceView* tex) { BlendMap->SetResource(tex); }
	void SetHeightMapSRV(ID3D11ShaderResourceView* tex) { HeightMap->SetResource(tex); }
...
public:
	...
	void SetCubeYunMap(ID3D11ShaderResourceView* cubemap)	{ CubeYunMap->SetResource(cubemap); }
	void SetRotateY(DirectX::CXMMATRIX M)					{ RotateY->SetMatrix(reinterpret_cast<const float*>(&M)); }
	void SetAngle(float angle)								{ Angle->SetFloat(angle); }

	ID3DX11EffectTechnique *SkyTech;
	ID3DX11EffectMatrixVariable *WorldViewProj;
	ID3DX11EffectShaderResourceVariable *CubeMap;
	ID3DX11EffectShaderResourceVariable *CubeYunMap;
	ID3DX11EffectMatrixVariable *RotateY;
	ID3DX11EffectScalarVariable *Angle;
};
#pragma endregion
...
{
public:
	BoardEffect(ID3D11Device* device, const std::wstring& filename);
	~BoardEffect();
	...
	void SetDirLights(const DirectionalLight* lights) { DirLights->SetRawValue(lights, 0, 3 * sizeof(DirectionalLight)); }
	void SetMaterial(const Material& mat) { Mat->SetRawValue(&mat, 0, sizeof(Material)); }
	void SetTextureMapArray(ID3D11ShaderResourceView* tex) { TextureMap->SetResource(tex); }
